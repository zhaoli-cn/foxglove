...
import com.zl.core.tool.function.CheckedFunction;
import com.zl.core.tool.utils.ClassUtil;
import com.zl.core.tool.utils.ConvertUtil;
import com.zl.core.tool.utils.ReflectUtil;
import com.zl.core.tool.utils.Unchecked;
...
@Slf4j
@AllArgsConstructor
public class ZiConverter implements Converter {
	...
	public Object convert(Object value, Class target, final Object fieldName) {
		...
		try {
			TypeDescriptor targetDescriptor = ZiConverter.getTypeDescriptor(targetClazz, (String) fieldName);
			// 1. 判断 sourceClazz 为 Map
			if (Map.class.isAssignableFrom(sourceClazz)) {
				return ConvertUtil.convert(value, targetDescriptor);
			} else {
				TypeDescriptor sourceDescriptor = ZiConverter.getTypeDescriptor(sourceClazz, (String) fieldName);
