package com.zl.core.tool.jackson;
...
public class MappingApiJackson2HttpMessageConverter extends AbstractReadWriteJackson2HttpMessageConverter {
	...
	private static ObjectMapper initWriteObjectMapper(ObjectMapper readObjectMapper) {
		...
		// null 处理
		writeObjectMapper.setSerializerFactory(writeObjectMapper.getSerializerFactory().withSerializerModifier(new ZiBeanSerializerModifier()));
		writeObjectMapper.getSerializerProvider().setNullValueSerializer(ZiBeanSerializerModifier.NullJsonSerializers.STRING_JSON_SERIALIZER);
