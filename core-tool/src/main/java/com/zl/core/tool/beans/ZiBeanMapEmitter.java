 ...
 * @author L.cm
 */
class ZiBeanMapEmitter extends ClassEmitter {
	private static final Type BEAN_MAP = TypeUtils.parseType(ZiBeanMap.class.getName());
	...
	public ZiBeanMapEmitter(ClassVisitor v, String className, Class type, int require) {
		super(v);
		...
		if (require != 0) {
			for (Iterator it = allProps.keySet().iterator(); it.hasNext(); ) {
				...
				if ((((require & ZiBeanMap.REQUIRE_GETTER) != 0) && !getters.containsKey(name)) ||
					(((require & ZiBeanMap.REQUIRE_SETTER) != 0) && !setters.containsKey(name))) {
					it.remove();
					getters.remove(name);
					setters.remove(name);
				}
